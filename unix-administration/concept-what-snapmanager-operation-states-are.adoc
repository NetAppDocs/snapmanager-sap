...
*Note:* You can split the clone that is created. When you stop the clone split operation you started and the operation is stopped successfully, the clone split operation state displays as failed.
...
If you want to delete the SnapManager lock file, navigate to `$ORACLE_HOME` on the target machine and delete the `sm_lock_ _TargetDBName_` file. After deleting the file, you must restart the SnapManager for SAP server.
